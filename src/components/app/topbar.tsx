...
/**
 * Top bar shared by both dashboards: page title, locale switch, account menu,
 * and the control that ends an admin impersonation session.
 */
export function AppTopbar({
  ...
  title: string;
  /** Where the account menu's settings item goes. */
  settingsHref: string;
  /** Where to land after ending an impersonation session. */
  homeHref: string;
  actions?: ReactNode;
}) {
  ...
  async function stopImpersonation() {
    ...
    router.push(`/${locale}${homeHref}`);
    router.refresh();
  }

  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b bg-background/95 px-4 backdrop-blur md:px-6">
      <h1 className="flex-1 truncate font-display text-lg font-semibold ltr:ml-12 md:ltr:ml-0 rtl:mr-12 md:rtl:mr-0">
        {title}
      </h1>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => router.push(`/${locale}${settingsHref}`)}>
            <User />
